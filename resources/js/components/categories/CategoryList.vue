    ...
    <div class="animated fadeIn">
        <b-card>
            <template slot="header" class="card-header">
                <span class="h3">
                    {{ $t('modules/categories.labels.titles.index') }}
                </span>
                <div
                    class="card-header-actions"
                    v-if="$acl.can('create categories')"
                >
                    <b-btn
                        ...
                        variant="primary"
                        v-b-tooltip.hover
                        :title="$t('modules/categories.buttons.create')"
                    >
                        <font-awesome-icon
                            icon="plus-circle"
                        ></font-awesome-icon>
...
export default {
    ...
    computed: {
        fields() {
            ...
            if (
                this.$acl.can('edit categories') ||
                this.$acl.can('delete categories')
            ) {
                fields.push({
                    key: 'actions',
