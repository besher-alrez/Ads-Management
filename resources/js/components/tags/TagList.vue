    ...
    <div class="animated fadeIn">
        <b-card>
            <template slot="header" class="card-header">
                <span class="h3">
                    {{ $t('modules/tags.labels.titles.index') }}
                </span>
                <div class="card-header-actions" v-if="$acl.can('create tags')">
                    <b-btn
                        :to="{ name: 'tags_create' }"
                        size="sm"
                        variant="primary"
                        v-b-tooltip.hover
                        :title="$t('modules/tags.buttons.create')"
                    >
                        <font-awesome-icon
                            icon="plus-circle"
                        ></font-awesome-icon>
            ...
            <b-datatable
                ...
                search-route="tags.search"
                delete-route="tags.destroy"
                :length-change="true"
                :paging="true"
                :infos="false"
                :export-data="false"
...
export default {
    ...
    computed: {
        fields() {
            ...
            if (this.$acl.can('edit tags') || this.$acl.can('delete tags')) {
                fields.push({
                    key: 'actions',
